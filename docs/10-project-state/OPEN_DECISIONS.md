...
| B7 | Security tooling, hosting, authentication mechanics | `docs/02-architecture/SECURITY_ARCHITECTURE.md`, `docs/02-architecture/IDENTITY_RECOMMENDATION.md` | Mechanics are Tier B; production access model/provider is escalated to Tier A because it handles real identity/financial data and may add cost or lock-in | Owner-only gateway is recommended for the next pilot; owner must approve audience, third-party handling and provider before implementation | Before production persistence | DECISION REQUIRED: YES |
