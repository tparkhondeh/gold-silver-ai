...
| B7 | Security tooling, hosting, authentication mechanics | `docs/02-architecture/SECURITY_ARCHITECTURE.md` | Standard security engineering practice; escalate if it involves handling real user credentials or PII beyond baseline secret hygiene | Partially — guarded local operator and public review boundary implemented; production identity/persistent-data controls remain | As needed, baseline by Phase 1 | Partially resolved |
