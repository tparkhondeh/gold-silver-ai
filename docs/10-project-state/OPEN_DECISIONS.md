...
| B7 | Security tooling, hosting, authentication mechanics | `docs/02-architecture/SECURITY_ARCHITECTURE.md`, `docs/02-architecture/IDENTITY_RECOMMENDATION.md`, `docs/09-operations/DEPLOYMENT.md` | Mechanics are Tier B; production access model/provider is escalated to Tier A because it handles real identity/financial data and may add cost or lock-in | Preliminary review recorded; owner must approve audience/provider, and the current server's Cloudflare-host override requires hosting-admin authorization before that candidate can pass preflight | Before production persistence | DECISION REQUIRED: YES |
