...
| A2 | Data source(s) / vendor(s) selection | `docs/05-data/DATA_SOURCES.md`, `docs/05-data/HISTORICAL_BACKFILL_PROPOSAL.md` | Cost, trust, and legal terms of use; every downstream number depends on this | Partially — the Phase 1 adapter set is accepted in ADR 0003; written Navasan storage scope and licensed Iranian redundancy remain open | Before operational analysis | Partially resolved; vendor purchase/contact/monitoring deferred to the owner's final API-integration stage |
...
| A5 | Single-user vs. multi-user | `docs/01-product/USER_REQUIREMENTS.md`, `docs/08-decisions/ADR/0006-owner-only-real-release.md` | Shapes access control and isolation | No | Owner-only next real release accepted; future multi-user expansion requires a new decision | Resolved for current scope |
...
| B2 | Data pipeline scheduling & manual-update tooling | `docs/05-data/DATA_PIPELINE.md` | Standard ingestion-engineering practice | No | Protected preview and PostgreSQL commit implemented; provider scheduling remains gated | Partially resolved |
...
| B4 | Historical data storage format, retention, backfill approach | `docs/05-data/HISTORICAL_DATA.md`, `docs/05-data/HISTORICAL_BACKFILL_PROPOSAL.md` | Append-only PostgreSQL and explicit gaps are recommended; exact retention/backfill remains bound to written vendor terms | No | Before backfill | Partially resolved; safe proposal prepared, no backfill authorized |
...
| B7 | Security tooling, hosting, authentication mechanics | `docs/02-architecture/SECURITY_ARCHITECTURE.md`, `docs/02-architecture/IDENTITY_RECOMMENDATION.md`, `docs/09-operations/DEPLOYMENT.md` | Mechanics are Tier B; provider/data handling is escalated to Tier A because it handles real identity/financial data and may add cost or lock-in | ADR 0006/0007 fix owner-only and minimum external login data; ADR 0008 defers exact provider during local/demo work but makes it mandatory before hosted real data. The current server's Cloudflare-host override still requires hosting-admin authorization if that candidate returns | Before hosted real data or production persistence | DECISION REQUIRED: YES at pre-real-data gate |
| B8 | Git hosting platform & branch-protection enforcement | `docs/00-governance/STABILITY_POLICY.md` | GitHub and the working-branch CI path are active; branch-protection verification remains | No | Before merge | Partially resolved |
...
| B11 | Monitoring tooling | `docs/09-operations/MONITORING.md` | Standard observability practice | No | Local health and strict readiness checks implemented; external probe/alerts before stable deployment | Partially resolved |
...
| B13 | Incident-response tooling | `docs/09-operations/INCIDENT_RESPONSE.md` | Standard practice | No | Local readiness/containment signal implemented; deployment rollback and alert evidence remain | Partially resolved |
