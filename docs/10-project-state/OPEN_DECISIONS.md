...
| B7 | Security tooling, hosting, authentication mechanics | `docs/02-architecture/SECURITY_ARCHITECTURE.md`, `docs/02-architecture/IDENTITY_RECOMMENDATION.md`, `docs/09-operations/DEPLOYMENT.md` | Mechanics are Tier B; provider/data handling is escalated to Tier A because it handles real identity/financial data and may add cost or lock-in | Owner-only audience and external identity limited to minimum login data are accepted in ADR 0006/0007; exact provider is unresolved, and the current server's Cloudflare-host override requires hosting-admin authorization before that candidate can pass preflight | Before production persistence | DECISION REQUIRED: YES |
