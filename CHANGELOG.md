...
- Added editable, explicit entry/exit/peer/cross/hold/wait/missing fixtures and
  Persian calculation/funding disclosures; no number is generated by AI.
- Defaulted market fetching and browser fallback to explicit opt-in; the local
  launcher keeps live/history network off. Preserved all existing adapters/keys.
- Fixed project PostgreSQL cold startup inheriting captured Windows pipes; local
  health, 16 integration tests and full 25-table backup/restore pass.
- Documented the source-linked engine map, remaining release dependencies and the
  owner's short test guide. Web tests now total 153; laboratory tests total 254.
...
- Added `asha.synthetic.parameter_freeze.v1` with exact laboratory-v1 factors,
  cutoffs, horizons, constraints and calculation rules. Real acceptance thresholds,
  stress magnitudes, evidence links and dataset identity stay null/empty; synthetic
...
- Added the exact canonical readiness reference and a read-only Persian panel in the
  local synthetic demo. The browser verifies the fixed fingerprint, ten ordered gates,
