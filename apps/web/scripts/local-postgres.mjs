...
async function verifyActivation(client) {
  ...
  const grants = await client.query(`SELECT bool_and(
    ...
    AND CASE
      WHEN c.relname IN ('instruments','sources','asha_schema_migrations','source_contract_versions','artifact_versions','dataset_observations','decision_records','decision_assumptions','decision_features','source_reconciliations','source_reconciliation_candidates')
  ...
  if (grants.rows[0]?.safe !== true || grants.rows[0]?.tables !== 19) throw new Error("Runtime table privileges do not match the least-privilege contract");
  ...
  const triggerNames = ["observations_are_immutable", "quarantine_records_are_immutable", "validation_results_are_immutable", "quarantine_resolutions_are_immutable", "ingestion_batches_are_immutable", "observation_contract_before_insert", "observations_cannot_be_truncated", "batches_cannot_be_truncated", "quarantine_cannot_be_truncated", "validations_cannot_be_truncated", "resolutions_cannot_be_truncated", "source_contract_versions_are_immutable", "artifact_versions_are_immutable", "dataset_observations_are_immutable", "decision_records_are_immutable", "decision_assumptions_are_immutable", "decision_features_are_immutable", "source_contract_versions_cannot_be_truncated", "artifact_versions_cannot_be_truncated", "dataset_observations_cannot_be_truncated", "decision_records_cannot_be_truncated", "decision_assumptions_cannot_be_truncated", "decision_features_cannot_be_truncated", "source_reconciliations_are_immutable", "source_reconciliation_candidates_are_immutable", "source_reconciliations_cannot_be_truncated", "source_reconciliation_candidates_cannot_be_truncated"];
