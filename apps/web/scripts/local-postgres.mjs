...
async function verifyActivation(client) {
  ...
  const grants = await client.query(`SELECT bool_and(
    ...
    WHERE n.nspname='public' AND c.relkind='r' AND c.relname IN
      ('instruments','sources','asha_schema_migrations','ingestion_batches','observations','quarantine_records','validation_results','quarantine_resolutions','user_portfolios','portfolio_holdings','portfolio_preferences','source_contract_versions','artifact_versions','dataset_observations','decision_records','decision_assumptions','decision_features','source_reconciliations','source_reconciliation_candidates','portfolio_transaction_events','portfolio_valuation_snapshots','portfolio_valuation_positions','portfolio_valuation_transactions')`);
  if (grants.rows[0]?.safe !== true || grants.rows[0]?.tables !== 23) throw new Error("Runtime table privileges do not match the least-privilege contract");
