...
import { PRIVATE_DATA_ROOT, planPrivateSupervision, assertPrivateReleaseDirectory, isEmptyPrivateCrontab } from "./private-supervision.ts";
...
let pool;
try {
  if (process.argv.slice(2).join(" ") !== "--activate-reviewed-release" || process.platform !== "linux" || process.getuid() === 0) throw Error();
  process.umask(0o077);
  ...
  // Never truncate/follow an existing lock; every later flock uses this verified file.
  const lock = await open(join(PRIVATE_DATA_ROOT, "service.lock"), constants.O_WRONLY | constants.O_CREAT | constants.O_EXCL | constants.O_NOFOLLOW, 0o600);
  await lock.close();
  ...
  const current = readCron(), plan = planPrivateSupervision(current, repository);
  ...
  process.stdout.write(JSON.stringify({ state: "project-supervision-installed", commit, previousCrontab: destination, restart: "boot-and-minute-recovery", backupSchedule: "daily-03:17-server-local", actualRebootTested: false }) + "\n");
