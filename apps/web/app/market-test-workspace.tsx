...
export function MarketTestWorkspace({ active, view, onNavigate }: { active: boolean; view: View; onNavigate: (view: View) => void }) {
  ...
  return <section className="view-stack market-test-workspace" data-testid="market-test-workspace" data-revision={portfolio.revision}>
    ...
    {(view === "overview" || view === "portfolio" || view === "market" || view === "data") && <section className="panel"><h3>سبد آزمون و منشأ قیمت</h3><div className="table-scroll"><table className="shared-table"><thead><tr><th>دارایی / واحد / عیار</th><th>موجودی فرضی</th><th>قیمت منبع</th><th>ارزش ثبت‌شده</th><th>وزن از کل با نقد</th><th>تازگی / منبع</th></tr></thead><tbody>{result?.rows.map((r) => <tr key={r.asset.id}><td><button className="text-button" onClick={() => { change({ selectedAsset: r.asset.id }); onNavigate("asset-center"); }}>{r.asset.name}</button> · {r.asset.unit === "gram" ? "گرم" : "عدد"} · {r.observation?.purityPermille ?? "عیار منبع نامشخص"}</td><td>{(r.quantityMilli / 1000).toLocaleString("fa-IR")}</td><td>{money(r.observation?.priceRial ?? null)}</td><td>{money(r.valueRial)}</td><td>{percent(result.weightsBps[r.asset.id])}</td><td>{labels[r.state]}{r.observation && <small className="market-provenance">نوسان · انتشار {date(r.observation.publishedAt)} · دریافت {date(r.observation.receivedAt)}</small>}</td></tr>)}</tbody></table></div><button className="text-button" onClick={() => onNavigate("portfolio")}>ویرایش موجودی و محدودیت‌های آزمون</button></section>}
