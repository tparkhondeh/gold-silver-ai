...
export function MarketTestWorkspace({ active, view, onNavigate }: { active: boolean; view: View; onNavigate: (view: View) => void }) {
  ...
  return <>
    ...
    <div className="market-test-boundary"><b>{portfolio.snapshot ? "قیمت واقعی بازار" : "قیمت بازار هنوز دریافت نشده"} · موجودی و نقد فرضیِ آزمون</b><span>منبع: نوسان · پلن رایگان با به‌روزرسانی دوساعتهٔ اعلام‌شده، نه خوراک لحظه‌ای · بدون اتصال به سبد شخصی یا موتور سفارش</span></div>
    ...
    {(view === "asset-center" || view === "analysis" || view === "risk") && <section className="panel"><h3>{selected?.asset.name}</h3><p data-testid="market-selected-summary">مقدار {selected ? <NumberValue value={selected.quantityMilli} denominator={1000} /> : "—"} · ارزش {money(selected?.valueRial ?? null)} · وزن {percent(result?.weightsBps[portfolio.selectedAsset] ?? null)} از ارزش ثبت‌شدهٔ کل، شامل نقد</p>{selected && <p>{labels[selected.state]} · {selected.observation ? `نوسان · انتشار ${date(selected.observation.publishedAt)} · دریافت ${date(selected.observation.receivedAt)}` : "قیمت و منبع موجود نیست"}</p>}{view === "asset-center" && selected && editor(selected.asset)}{view === "analysis" && <><h4>کوتاه‌مدت: {<NumberValue value={portfolio.shortDays} />} روز · میان‌مدت: {<NumberValue value={portfolio.mediumDays} />} روز</h4><p>روند، نوسان، حباب، اهداف و امتیاز تصمیم بدون ورودی واقعی لازم قابل محاسبه نیستند؛ وضعیت ارزش‌گذاری در بالای صفحه مشخص است.</p></>}{view === "risk" && <><p>حد نقد فرضی: {percent(portfolio.minimumCashBps)}؛ مبلغ متناظر با ارزش ثبت‌شده: {money(result?.minimumCashRial ?? null)}.</p><p>وزن نقد: {percent(result?.cashWeightBps ?? null)}؛ حداکثر وزن مجاز ورودی: {percent(portfolio.maximumAssetBps)}.</p>{isAssetWeightAboveLimit(selected?.valueRial, result?.observedTotalRial, portfolio.maximumAssetBps) === true && <p role="alert">وزن دارایی از حد واردشده بیشتر است؛ این هشدار عددی است، نه دستور فروش.</p>}<p>نوسان، افت، هزینهٔ خروج و ریسک بحران: دادهٔ کافی وجود ندارد.</p></>}</section>}
