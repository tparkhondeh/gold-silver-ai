...
const number = (value: number) => <NumberValue value={value} />;
...
export function SharedAnalysisPanel({ report, selectedAssetId, view }: { report: Report; selectedAssetId: string; view: View }) {
  return <section className="panel" data-testid="shared-diagnostics" data-revision={report.source.revision}>
    ...
    {(view === "market" || view === "analysis") && <div data-testid="shared-metal-ratio"><b>نسبت قیمت یک گرم طلای خالص به یک گرم نقرهٔ خالص: </b>{report.metalRatio ? <NumberValue value={report.metalRatio.numerator} denominator={report.metalRatio.denominator} /> : "قابل محاسبه نیست"}<p>تعدیل عیار ۷۵۰ و ۹۹۹ انجام شده؛ نسبت بدون واحد است، نه حباب و نه دستور تبدیل.</p>{report.metalRatioMissing && <p role="alert">{report.metalRatioMissing}</p>}{report.metalRatio && <details><summary>محاسبهٔ دقیق نسبت</summary><code dir="ltr">{report.metalRatio.numerator} / {report.metalRatio.denominator}</code><p>قیمت طلا ÷ عیار طلا، تقسیم بر قیمت نقره ÷ عیار نقره؛ نمایش حداکثر یک اعشار است؛ کسر دقیق حفظ شده است.</p></details>}</div>}
    {(view === "data" || view === "market") && <div className="table-scroll"><table className="shared-table"><thead><tr><th>دارایی</th><th>وضعیت قیمت</th><th>فاصلهٔ خرید/فروش از مبنا</th><th>منشأ / اعتبار</th></tr></thead><tbody>{report.quotes.map((quote) => <tr key={quote.assetId}><td>{report.source.inputSnapshot.assets.find((asset) => asset.id === quote.assetId)!.name}</td><td>{quote.state === "usable" ? "معتبر در تاریخ ساختگی بررسی" : quote.issues.map((issue) => quoteIssue[issue]).join("؛ ")}</td><td>{quote.spread ? <NumberValue value={BigInt(quote.spread.numerator) * 100n} denominator={quote.spread.denominator} unit="٪" /> : "قابل محاسبه نیست"}</td><td>ورودی ساختگی سبد · {quote.quotedOn} تا {quote.validUntil}</td></tr>)}</tbody></table><p>فاصله = (قیمت خرید − قیمت فروش) ÷ قیمت مبنا. این عدد کل هزینهٔ معامله نیست.</p></div>}
