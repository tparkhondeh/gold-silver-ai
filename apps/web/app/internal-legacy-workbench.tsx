...
// Retained for internal regression tests only; never imported by the public page.
...
import { NumberValue, MoneyValue } from "./number-value";
import { formatNumber } from "./number-display";
...
export default function Home() {
  ...
  const [legacyHoldings, setHoldings] = useState<Holding[]>([]);
  const [purchaseBook, setPurchaseBook] = useState<PurchaseBook>(emptyPurchaseBook);
  const [purchaseStorageIssue, setPurchaseStorageIssue] = useState(false);
  const [portfolioMode, setPortfolioMode] = useState<"personal" | "demo">("personal");
  const purchaseEvaluation = useMemo(() => {
    try { return evaluatePurchaseBook(purchaseBook, legacyHoldings); }
    catch { return { ...evaluatePurchaseBook(emptyPurchaseBook()), holdings: legacyHoldings, projectionIssues: ["ورودی قدیمی نیازمند اصلاح است؛ اصل آن حفظ شد و تجمیع متوقف است."] }; }
  }, [purchaseBook, legacyHoldings]);
  const holdings = portfolioMode === "personal" ? purchaseEvaluation.holdings : legacyHoldings;
  function holdingPurchaseDate(holding: Holding) {
    const aggregate = purchaseEvaluation.assets.find((asset) => asset.id === holding.id);
    return aggregate ? `${formatNumber(aggregate.lotIds.length)} خرید با تاریخ جدا در دفتر${aggregate.sourceLegacyIds.length ? "؛ به‌علاوهٔ موجودی قبلی" : ""}` : formatPurchaseDate(holding.purchaseDate);
  }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      try {
        const savedBook = sessionStorage.getItem("asha-purchase-book-v1");
        if (savedBook) setPurchaseBook(validatePurchaseBook(JSON.parse(savedBook)));
      } catch { setPurchaseStorageIssue(true); }
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem("gold-silver-holdings", JSON.stringify(legacyHoldings));
    sessionStorage.setItem("gold-silver-portfolio-mode", portfolioMode);
  }, [legacyHoldings, holdingsLoaded, portfolioMode, marketTestActive, legacyStorageIssue]);
  ...
  useEffect(() => {
    ...
    for (const quote of feed.quotes) {
      ...
      if (previous && previous.status === "valid" && previous.currency === quote.currency && previous.unit === quote.unit && Number.isFinite(currentObservedAt) && Number.isFinite(previousObservedAt) && currentObservedAt > previousObservedAt) {
        ...
        if (elapsedMs <= 24 * 60 * 60_000 && Math.abs(changePercent) >= threshold) {
          ...
          const windowText = elapsedMinutes < 60 ? `${formatNumber(elapsedMinutes)} دقیقه` : `${formatNumber(Math.round(elapsedMinutes / 60))} ساعت`;
          incoming.push({
            id: `volatility-${quote.instrumentCode}-${quoteObservedAt(quote)}`,
            kind: "volatility",
            title: `نوسان شدید ${instrumentName}`,
            message: `${changePercent > 0 ? "رشد" : "افت"} ${formatNumber(Math.abs(changePercent))}٪ در ${windowText}؛ از ${formatQuote(previous, alertUsdTomanRate)} به ${formatQuote(quote, alertUsdTomanRate)}.`,
    ...
    if (staleCount > 0) incoming.push({
      ...
      message: `${formatNumber(staleCount)} قیمت از حد تازگی عبور کرده و وارد ارزش‌گذاری یا هشدار فرصت نمی‌شود.`,
      createdAt: feed.collectedAt,
      read: false,
      demo: false,
    });
  ...
  function openEditHolding(holding: Holding) {
    if (portfolioRequestRef.current) return;
    if (!legacyHoldings.some((item) => item.id === holding.id)) { setView("portfolio"); return; }
  ...
  async function savePersonalPortfolioToDatabase() {
    ...
    if (purchaseStorageIssue || JSON.stringify(purchaseBook) !== JSON.stringify(snapshot.purchaseBook ?? emptyPurchaseBook())) {
      setPortfolioPersistence({ state: "error", snapshot, message: "دفتر خرید با نسخهٔ دیتابیس یکسان نیست؛ پیش از ذخیره، نسخهٔ دیتابیس را بازیابی کن. اطلاعات فعلی حذف نشد." });
      return;
    }
    const controller = new AbortController();
    portfolioRequestRef.current = controller;
    ...
    try {
      const response = await fetch("/api/portfolio", {
        ...
        body: JSON.stringify({ expectedVersion: snapshot.version, holdings: legacyHoldings, preferences, purchaseBook }),
  ...
  async function restorePersonalPortfolioFromDatabase() {
    ...
    try {
      ...
      const restoredBook = snapshot.purchaseBook ?? emptyPurchaseBook();
      setPurchaseBook(restoredBook);
      try { sessionStorage.setItem("asha-purchase-book-v1", JSON.stringify(restoredBook)); setPurchaseStorageIssue(false); }
      catch { setPurchaseStorageIssue(true); }
  ...
  async function commitPurchaseBook(next: PurchaseBook) {
    if (portfolioMode !== "personal" || marketTestActive || portfolioRequestRef.current || purchaseStorageIssue
      || portfolioPersistence.state === "checking" || portfolioPersistence.state === "unavailable") {
      throw new Error("ثبت خرید به اتصال دیتابیس محلی و بازیابی سالم سبد نیاز دارد؛ هیچ ردیفی اضافه نشد.");
    }
    const snapshot = portfolioPersistence.snapshot;
    if (JSON.stringify(purchaseBook) !== JSON.stringify(snapshot.purchaseBook ?? emptyPurchaseBook())) {
      throw new Error("دفتر خرید با نسخهٔ ذخیره‌شده یکسان نیست؛ ابتدا «بازیابی نسخهٔ دیتابیس» را بزن.");
    }
    const validated = validatePurchaseBook(next);
    const preferences = { ...ownerConstraints, analysisHorizon, decisionHorizon };
    const controller = new AbortController();
    portfolioRequestRef.current = controller;
    setPortfolioPersistence({ state: "saving", snapshot, message: "در حال ثبت یکپارچهٔ خریدها؛ نتیجه هنوز تأیید نشده…" });
    let failureMessage = "نتیجهٔ ثبت نامشخص است؛ ورودی و پیش‌نمایش حفظ شدند. پیش از تکرار، نسخهٔ دیتابیس را بازیابی کن.";
    try {
      const response = await fetch("/api/portfolio", {
        method: "PUT", headers: { "Content-Type": "application/json", "X-Asha-Portfolio-Request": "save" },
        body: JSON.stringify({ expectedVersion: snapshot.version, holdings: legacyHoldings, preferences, purchaseBook: validated }),
        signal: AbortSignal.any([controller.signal, AbortSignal.timeout(10_000)]),
      });
      const payload = await response.json();
      if (controller.signal.aborted) throw new Error("نتیجهٔ ثبت نامشخص است؛ پیش از تکرار، نسخهٔ دیتابیس را بازیابی کن.");
      if (!response.ok || payload.ok !== true) {
        failureMessage = response.status === 409 ? "نسخهٔ ذخیره‌شده تغییر کرده است؛ ابتدا بازیابی کن. هیچ ردیفی به این نما اضافه نشد."
          : response.status === 422 ? "ورودی خرید یا موجودی قدیمی معتبر نیست؛ ثبت انجام نشد. خطا را اصلاح و دوباره پیش‌نمایش کن." : failureMessage;
        throw new Error(failureMessage);
      }
      const saved = decodePortfolioSnapshot(payload.snapshot);
      if (JSON.stringify(saved.purchaseBook) !== JSON.stringify(validated)) throw new Error("پاسخ ذخیره با درخواست برابر نیست؛ پیش از ادامه بازیابی کن.");
      setPurchaseBook(validated);
      try { sessionStorage.setItem("asha-purchase-book-v1", JSON.stringify(validated)); }
      catch { setPurchaseStorageIssue(true); }
      setPortfolioPersistence({ state: "ready", snapshot: saved, message: "خریدها و سبد یک‌جا در دیتابیس محلی ثبت شدند؛ میانگین‌ها از ردیف‌های اصلی محاسبه می‌شوند." });
    } catch {
      if (!controller.signal.aborted) setPortfolioPersistence({ state: "error", snapshot, message: failureMessage });
      throw new Error(failureMessage);
    } finally { if (portfolioRequestRef.current === controller) portfolioRequestRef.current = null; }
  }
  ...
  function activateDemoPortfolio(destination: View) {
    if (holdingsLoaded && !legacyStorageIssue && portfolioMode !== "demo" && !sessionStorage.getItem("asha-personal-holdings-backup-v1")) sessionStorage.setItem("asha-personal-holdings-backup-v1", JSON.stringify(legacyHoldings));
  ...
  return (
    <div className="app-shell">
      ...
      <main className="workspace" id="top">
        ...
        {!marketTestActive && portfolioMode === "personal" && purchaseStorageIssue && <p className="action-error" role="alert">نسخهٔ مرورگر دفتر خرید قابل اتکا نیست؛ اصل آن حفظ شد. برای ادامه از بخش دارایی‌ها، نسخهٔ دیتابیس را بازیابی کن.</p>}
        ...
        <div className="page-content">
                ...
                <article className="panel asset-center-card asset-decision-card">
                  ...
                  {portfolioMode === "demo" && selectedSandboxIntelligence ? <>
                    <div className="asset-decision-list">
                      <div className="asset-decision-item"><div><small>تصمیم همگن · همین کلاس</small><span>امتیاز {<NumberValue value={selectedSandboxIntelligence.score} />}</span></div><strong>{selectedSandboxIntelligence.homogeneousDecision.label}</strong><p>{selectedSandboxIntelligence.homogeneousDecision.reason}</p></div>
          ...
          {view === "analysis" && !personalProjectionBlocked && <section className="view-stack">
            ...
            <section className="analysis-summary-grid">
              <article><small>{portfolioMode === "demo" ? "قیمت ساختگی" : "قیمت تازه"}</small><strong>{<NumberValue value={effectiveLiveQuoteCount} />}</strong><p>{portfolioMode === "demo" ? "۰ رکورد منقضی در مجموعهٔ تمرینی" : `${formatNumber(staleQuoteCount)} رکورد منقضی`}</p></article>
              <article><small>پوشش ارزش‌گذاری سبد</small><strong>{<NumberValue value={valuedHoldingCount} />} / {<NumberValue value={holdings.length} />}</strong><p>{portfolioRateStatus}</p></article>
              <article><small>حباب قابل محاسبه</small><strong>{<NumberValue value={bubbleAvailableCount} />}</strong><p>{portfolioMode === "demo" ? `${sandboxPremiumMethodology.windowLabel} فعال` : "آمار تاریخی هنوز قفل است"}</p></article>
          ...
          {view === "risk" && <section className="view-stack">
            ...
            <section className="risk-grid">{portfolioMode === "demo" ? <><article><h3>تمرکز سبد</h3><p>{largestAllocation ? `${largestAllocation.holding.name} بزرگ‌ترین موقعیت سبد است.` : "—"}</p><b>{largestAllocationPercent === null ? "—" : `${formatNumber(largestAllocationPercent)}٪ از سبد`}</b></article><article><h3>بازده نمایشی</h3><p>اختلاف ارزش ساختگی امروز با بهای خرید نمونه.</p><b>{portfolioProfitPercent === null ? "—" : `${formatNumber(portfolioProfitPercent)}٪`}</b></article><article><h3>نقدشوندگی نمونه</h3><p>برچسب آزمایشی برای بررسی نمایش ریسک خروج.</p><b>متوسط · ساختگی</b></article><article><h3>سناریوی فشار</h3><p>اثر یک شوک یکسان روی کل ارزش سبد نمونه.</p><b>{<NumberValue value={Number(stressMove)} />}٪</b></article></> : <><article><h3>تمرکز سبد</h3><p>وزن هر دارایی و وابستگی بیش‌ازحد به یک بازار.</p><b>نیازمند ارزش روز کامل</b></article><article><h3>نوسان و افت</h3><p>نوسان، افت تاریخی و زمان بازیابی در دادهٔ ایران.</p><b>نیازمند تاریخچه معتبر</b></article><article><h3>نقدشوندگی</h3><p>اثر فاصلهٔ خرید و فروش و امکان خروج در فشار بازار.</p><b>نیازمند دادهٔ بازار</b></article><article><h3>سناریوی فشار</h3><p>ارز، اونس، پریمیوم داخلی و شوک‌های هم‌زمان.</p><b>نیازمند مدل مصوب</b></article></>}</section>
            {portfolioMode === "demo" ? <div className="split-grid analysis-grid"><section className="panel allocation-panel"><div className="panel-head"><SectionTitle eyebrow="DEMO ALLOCATION" title="ترکیب سبد نمایشی"/><b className="score">جمع ۱۰۰٪</b></div><div className="allocation-list">{allocationRows.map((row) => { const weight = row.value !== null && portfolioMarketValue ? Math.round((row.value / portfolioMarketValue) * 100) : 0; return <div key={row.holding.id}><div><span>{row.holding.name}</span><b>{<NumberValue value={weight} />}٪</b></div><span className="allocation-track"><i style={{ width: `${weight}%` }}/></span></div>; })}</div></section><section className="panel scenario-card"><h3>آزمایش فشار سبد</h3><p>درصد شوک فرضی را تغییر بده و نتیجه را فوراً ببین.</p><label>شوک یکسان به سبد<input type="range" min="-50" max="0" step="1" value={stressMove} onChange={(event) => setStressMove(event.target.value)}/><span className="negative">{<NumberValue value={Number(stressMove)} />}٪</span></label><div className="scenario-presets" aria-label="سناریوهای فشار سریع">{[-10, -20, -30, -40].map((move) => <button type="button" className={stressMove === String(move) ? "active" : ""} key={move} onClick={() => setStressMove(String(move))}>{<NumberValue value={move} />}٪</button>)}</div><div className="scenario-result"><small>ارزش پس از شوک نمایشی</small><strong>{stressScenarioValue === null ? "—" : <MoneyValue value={stressScenarioValue} usdTomanRate={portfolioUsdTomanRate} />}</strong><p>{stressScenarioValue === null || portfolioMarketValue === null ? "سبد نمایشی را فعال کن." : `افت فرضی: ${formatPortfolioMoney(stressScenarioValue - portfolioMarketValue)}`}</p></div></section></div> : <section className="panel"><EmptyLock title="ابزارهای ریسک آمادهٔ تجربه‌اند" text="با فعال‌کردن سبد نمایشی، وزن دارایی‌ها، تمرکز، بازده نمونه و سناریوی فشار قابل استفاده می‌شوند؛ حالت واقعی همچنان قفل می‌ماند."/></section>}
          ...
          {view === "data" && <section className="view-stack">
            ...
            <section className="data-cards">
              <article><small>منابع در دسترس</small><strong>{<NumberValue value={(portfolioMode === "demo" ? 1 : connectedSourceCount)} />}</strong><p>{portfolioMode === "demo" ? "فقط آزمایشگاه ساختگی اشا" : "API، خوراک موقت و Snapshot دستی تفکیک می‌شوند."}</p></article>
            ...
            <section className="panel engine-readiness"><div className="panel-head"><SectionTitle eyebrow="OPERATIONAL READINESS" title="وضعیت موتورهای اشا" text="آماده‌بودن رابط با آماده‌بودن تصمیم مالی یکی نیست؛ وضعیت واقعی و آزمایشی هرگز با هم ادغام نمی‌شوند."/><a className="source-action" href="/api/health" target="_blank" rel="noreferrer">خروجی سلامت واقعی JSON ↗</a></div>
              <div className="engine-readiness-grid">{portfolioMode === "demo" ? <>
                ...
                <article className="demo-ready"><span>خوراک بازار</span><b>{<NumberValue value={sandboxQuotes.length} />} قیمت ساختگی</b><small>{sandboxMethodology.datasetId}</small></article>
                <article className="demo-ready"><span>دادهٔ ایران</span><b>{<NumberValue value={effectiveFreshIranQuoteCount} />} رکورد ساختگی</b><small>بدون ادعای تازگی بازار</small></article>
                ...
                <article className={liveQuoteCount > 0 ? "ready" : "blocked"}><span>خوراک بازار</span><b>{liveQuoteCount > 0 ? `${formatNumber(liveQuoteCount)} قیمت تازه` : "بدون قیمت تازه"}</b><small>منبع، زمان و تازگی کنترل می‌شود</small></article>
                ...
                <article className="blocked"><span>تصمیم مالی</span><b>قفل ایمنی</b><small>{<NumberValue value={decisionReadiness.passedCount} />} از ۶ دروازه آماده</small></article>
              </>}</div>
            </section>
...
function MarketTable({ rows, quotes, usdTomanRate }: { rows: Instrument[]; quotes: Map<string, LiveQuote>; usdTomanRate: number | null }) {
  ...
  return <div className="market-table"><div className="market-row market-head"><SortButton label="نماد" active={sort.key === "instrument"} direction={sort.direction} onClick={() => toggleSort("instrument")}/><SortButton label="بازار" active={sort.key === "market"} direction={sort.direction} onClick={() => toggleSort("market")}/><SortButton label="آخرین قیمت (تومان · دلار)" active={sort.key === "price"} direction={sort.direction} onClick={() => toggleSort("price")}/><SortButton label="تازگی" active={sort.key === "freshness"} direction={sort.direction} onClick={() => toggleSort("freshness")}/><SortButton label="منشأ" active={sort.key === "source"} direction={sort.direction} onClick={() => toggleSort("source")}/></div>{sortedRows.map((row) => {
    ...
    return <div className="market-row" key={row.code}><span className="instrument"><i className={row.tone}>{row.icon}</i><span><strong>{row.name}</strong><small>{row.code}</small></span></span><span>{row.market}</span>{quote?.status === "valid" ? <strong className="live-value">{<QuoteNumber quote={quote} rate={usdTomanRate} />}</strong> : quote ? <strong className="no-data">منقضی؛ قیمت جاری ناموجود</strong> : <strong className="no-data">—</strong>}{quote ? <span className={quote.status === "valid" ? "fresh" : "pending"}>{formatFreshness(quote.publishedAt ?? quote.collectedAt)}</span> : <span className="pending">در انتظار</span>}{quote ? quote.sourceId === "asha-sandbox" ? <span className="source-link sandbox-source-label"><span>{quote.sourceName}</span><small>ساختگی · نسخه‌دار</small></span> : <a className="source-link" href={quote.sourceUrl} target="_blank" rel="noreferrer"><span>{quote.sourceName}</span><small>{sourceQualityLabel(quote.quality)}</small></a> : <span className="source-none">تعریف نشده</span>}</div>;
  })}</div>;
}
...
function BubbleTable({ rows, demoMode }: { rows: Array<{ holding: Holding; applicable: boolean; current: number | null; minimum: number | null; average: number | null; maximum: number | null }>; demoMode: boolean }) {
  ...
  const metricText = (value: number | null, applicable: boolean) => value !== null ? <NumberValue value={value} unit="٪" /> : demoMode && !applicable ? "نامرتبط" : "نیازمند تاریخچه";
  return <div className="bubble-table"><div className="bubble-row bubble-head"><SortButton label="دارایی" active={sort.key === "name"} direction={sort.direction} onClick={() => toggleSort("name")}/><SortButton label="حباب فعلی" active={sort.key === "current"} direction={sort.direction} onClick={() => toggleSort("current")}/><SortButton label="کمترین حباب" active={sort.key === "minimum"} direction={sort.direction} onClick={() => toggleSort("minimum")}/><SortButton label="میانگین حباب" active={sort.key === "average"} direction={sort.direction} onClick={() => toggleSort("average")}/><SortButton label="بیشترین حباب" active={sort.key === "maximum"} direction={sort.direction} onClick={() => toggleSort("maximum")}/></div>{sortedRows.map((row) => <div className="bubble-row" key={row.holding.id}><b>{row.holding.name}</b><span className={row.current === null ? "no-data" : row.current < 0 ? "negative" : "positive"}>{row.current === null ? demoMode && !row.applicable ? "نامرتبط" : "قابل محاسبه نیست" : <NumberValue value={row.current} unit="٪" />}</span><span className={row.minimum === null ? "history-pending" : "sandbox-history"}>{metricText(row.minimum, row.applicable)}</span><span className={row.average === null ? "history-pending" : "sandbox-history"}>{metricText(row.average, row.applicable)}</span><span className={row.maximum === null ? "history-pending" : "sandbox-history"}>{metricText(row.maximum, row.applicable)}</span></div>)}</div>;
}

function QuoteNumber({ quote, rate }: { quote: LiveQuote; rate: number | null }) {
  return quote.currency === "USD" ? <><NumberValue value={quote.value} unit="دلار" /> · {isUsableUsdTomanRate(rate) ? <MoneyValue value={quote.value * rate} /> : "معادل تومانی نامشخص"}</> : <MoneyValue value={quote.value} usdTomanRate={rate} />;
}
