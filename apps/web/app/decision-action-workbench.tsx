...
export function DecisionActionWorkbench({ input: sharedInput, onInputChange, blockedReason, onSave, onRestore }: WorkbenchProps = {}) {
  ...
  return <section className="panel action-workbench" aria-labelledby="action-workbench-title" data-testid="decision-action-workbench">
    ...
    <details className="action-detail"><summary>سناریوهای ساختگیِ آزمون</summary><p>انتخاب سناریو، ورودی فعلی نمونه را تغییر می‌دهد؛ نسخهٔ ذخیره‌شده دست‌نخورده می‌ماند.</p><div className="action-scenarios" aria-label="سناریوی آزمون">{actionScenarios.map((scenario) => <button type="button" key={scenario} aria-pressed={input.scenario === scenario} className={input.scenario === scenario ? "active" : ""} onClick={() => { setInput(buildActionFixture(scenario)); setView("final"); setNotice(""); }}>{scenarioLabels[scenario]}</button>)}</div></details>
