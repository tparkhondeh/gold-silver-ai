...
export function FileMarketWorkspace({ view, onNavigate }: { view: View; onNavigate: (view: View) => void }) {
  ...
  return <section className="view-stack market-test-workspace" data-testid="file-market-workspace" data-revision={portfolio.inputs.revision}>
    <div className="market-test-boundary"><b>آزمون اتصال فایل — قیمت، موجودی و نقد کاملاً ساختگی</b><span>این قالب نمونهٔ پروژه است؛ قالب واقعی رهاورد هنوز تأیید نشده است.</span></div>
    ...
    {(view === "overview" || view === "portfolio" || view === "data" || view === "market") && <section className="panel"><h3>قیمت‌های فایل و موجودی فرضی</h3><div className="table-scroll"><table className="shared-table"><thead><tr><th>دارایی / واحد / عیار</th><th>مقدار فرضی</th><th>قیمت</th><th>ارزش</th><th>وزن از کل با نقد</th><th>تازگی / منشأ</th></tr></thead><tbody>{result?.rows.map(r => <tr key={r.asset.id}><td><button className="text-button" onClick={() => { change({ selectedAsset: r.asset.id }); onNavigate("asset-center"); }}>{r.asset.name}</button> · {r.asset.unit === "gram" ? "گرم" : "عدد"} · {r.observation?.purityPermille ?? "عیار نامشخص"}</td><td>{quantity(r.asset.id)}</td><td>{money(r.observation?.priceRial ?? null)}</td><td>{money(r.valueRial)}</td><td>{percent(result.weightsBps[r.asset.id])}</td><td>{states[r.state]}<small className="market-provenance">فایل ساختگی · ردیف {r.observation?.line ?? "—"} · زمان {r.observation?.publishedAt ?? "نامشخص"}</small></td></tr>)}</tbody></table></div></section>}
    ...
    {(view === "asset-center" || view === "analysis" || view === "risk") && <section className="panel"><h3>{selected?.asset.name}</h3><p data-testid="file-selected-summary">مقدار {selected ? quantity(selected.asset.id) : "—"} · ارزش {money(selected?.valueRial ?? null)} · وزن {percent(result?.weightsBps[portfolio.inputs.selectedAsset])} از کل با نقد</p>{selected && <p>{states[selected.state]}</p>}{view === "asset-center" && selected && editor(selected.asset)}<p>کوتاه‌مدت {portfolio.inputs.shortDays} روز · میان‌مدت {portfolio.inputs.mediumDays} روز</p>{view === "risk" && <><p>حد نقد {percent(portfolio.inputs.minimumCashBps)}؛ معادل {money(result?.minimumCashRial ?? null)}. وزن نقد {percent(result?.cashWeightBps)}.</p><p>سقف وزن {percent(portfolio.inputs.maximumAssetBps)}؛ {selected && (result?.weightsBps[selected.asset.id] ?? 0) > portfolio.inputs.maximumAssetBps ? "وزن بیشتر از حد فرضی است؛ دستور فروش نیست." : "این حدود صرفاً ورودی آزمون هستند."}</p></>}<p>روند، نوسان، حباب و ریسک بحران از فایل تک‌قیمتی قابل استنتاج نیستند.</p></section>}
    {(view === "overview" || view === "decisions" || view === "agents") && <section className="panel"><h3>نتیجه: تصمیم‌ناپذیر</h3><p>برای هر دو افق، مقدار، مبلغ، هزینه، شرط اقدام، منبع تأمین و نقد بعد تعیین نشده‌اند؛ هیچ بودجه‌ای مصرف نمی‌شود و «بدون تغییر» صادر نشده است.</p><p>کوتاه‌مدت {portfolio.inputs.shortDays} روز · میان‌مدت {portfolio.inputs.mediumDays} روز</p><details className="action-detail"><summary>دلیل و محدودیت</summary>{result?.decision.reasons.map(reason => <p key={reason}>{reason}</p>)}</details></section>}
    <details className="action-detail"><summary>ردپای فایل و نگهداری</summary><p>فایل به سرور یا API ارسال نمی‌شود؛ فقط همین تصویر ساختگی در فضای جداگانه مرورگر ذخیره می‌شود. اثر انگشت برای تطبیق بایت‌هاست، نه اثبات اصالت فروشنده.</p><code dir="ltr">{FILE_PROFILE_VERSION}</code><p dir="ltr">SHA-256: {portfolio.file?.fileSha256 ?? "—"}</p><p>دریافت محلی: {portfolio.file?.receivedAt ?? "—"} · اندازه: {portfolio.file?.fileBytes ?? 0} بایت</p><p>ریال/تومان از ستون صریح فایل؛ ضریب یک. مبلغ کسری موجودی مطابق روش قبلی رو به پایین تا یک ریال محاسبه می‌شود. بازبینی {portfolio.inputs.revision}.</p></details>
  </section>;
}
