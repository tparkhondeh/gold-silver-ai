...
test("private configuration is exact, owner-bound, least-privilege local DB and never prints input", async () => {
  ...
  assert.deepEqual(identityBackupExclusions, ["private_owner_login_transactions", "private_owner_sessions", "private_passkey_owners", "private_passkey_credentials", "private_passkey_bootstrap_grants", "private_passkey_challenges"].map(table => `--exclude-table-data=*.${table}`));
});
